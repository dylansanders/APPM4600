...
def driver():
    ...
    Nmax = 100
    x0= np.array([0.31491335,-0.8364749,1.28359035])
    tol = 1e-6
...
def evalF(x):
    ...
    F[1] = ((1-x[0])**(.25))+x[1]+(0.05*x[2])-(0.15*x[2])
...
def evalJ(x): 
    ...
    J = np.array([[x[0]-(x[1]*x[2]*np.sin(x[0]*x[1]*x[2])),-x[0]*x[2]*np.sin(x[0]*x[1]*x[2]),-x[0]*x[1]*np.sin(x[0]*x[1]*x[2])], 
        [-1/(4*(1-x[0])**(.75)),1,(0.1*x[2])-0.15],[-2*x[0],(-0.2*x[1])+0.01,1]])
    return J
